...
def generate_benchmark(
    ...
    def sample_per_class(c):
        ...
        indices_c = indices_per_class[c][
            torch.randperm(len(indices_per_class[c]))
        ]
    ...
    def create_dataset_exp_i(exp_i):
        ...
        all_indices_i = torch.cat(
            [selected_indices[c.item()][exp_i] for c in present_classes]
        )
    ...
    benchmark.n_samples_per_exp = [
        len(benchmark.samples_per_exp[i])
        for i in range(scenario_table.shape[1])
    ]
    ...
    benchmark.present_classes_in_each_exp = [
        torch.where(benchmark.scenario_table[:, i])[0] for i in range(n_e)
    ]
    ...
    benchmark.seen_classes = [
        classes_seen_sofar(i)
        for i in range(len(benchmark.present_classes_in_each_exp))
    ]
